...
int main (int argc, char** argv)
{
    ...
    int blocklens[2] = {1, 1};
    MPI_Aint displs[2] = {0, sizeof(double)};
    MPI_Datatype types[2] = {MPI_DOUBLE, MPI_DOUBLE};
    MPI_Type_create_struct(2, blocklens, displs, types, &MPI_Complex);
    ...
    //     // n = 2 * n;
        ...
        stime1 = MPI_Wtime();
        MPI_Datatype MPI_W_FullArray;
        MPI_Type_contiguous(n/2, MPI_Complex, &MPI_W_FullArray); // n必须为偶数
        MPI_Type_commit(&MPI_W_FullArray);
		MPI_Bcast(w, 1, MPI_W_FullArray, 0, MPI_COMM_WORLD);
        ...
        MPI_Scatter(para_x, local_n, MPI_Complex,  // 使用新数据类型
                local_x, local_n, MPI_Complex, 
                0, MPI_COMM_WORLD);
		ptime1 = MPI_Wtime();
		int mj = cfft2(local_n, local_x, local_y, w, sgn);
		
		MPI_Gather(local_y, local_n, MPI_Complex,  // 使用新数据类型
               para_y, local_n, MPI_Complex, 
               0, MPI_COMM_WORLD);
        int cnt = 0;
        bool tagl = 0;
        if(rank == 0){
